...
1. Connect to Ollama (`qwen3.5:9b`) via local HTTP API.
...
- Local models detected, including `qwen3.5:9b`.

### C) Real-model structured tool-call loop

Command used:
- `python3 orchestrator/main_orchestrator.py --workspace-root "$PWD" --task "Use the dummy_sandbox_echo tool on relative path docs, then say DONE." --model qwen3.5:9b`
